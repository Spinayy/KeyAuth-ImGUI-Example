...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    text_selected->second = ImClamp(text_selected->second + 0.8f * ImGui::GetIO().DeltaTime * (*v ? 7.f : -7.2f), -0.2f, 1.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    ImU32 check_col = ImColor(0.646f, 0.f, 0.855f, 1.0f * text_selected->second * 4.f);

    /*window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 0.8f), check_bb.Max - ImVec2(3, 4), ImColor(25, 25, 25, 255), 2.f);
    window->DrawList->AddRect(check_bb.Min + ImVec2(1, 0.8f), check_bb.Max - ImVec2(2, 2), ImColor(3, 3, 3), 2.f);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1.2f, 1.0f), check_bb.Max - ImVec2(3, 4), check_col, 2.f);*/
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 1, check_bb.Min.y + 1 + ((check_bb.Max.y - check_bb.Min.y) / 2) - label_size.y / 2 - 1), label);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 frame_size = ImVec2(size.x, ImMax(size.y + 31, label_size.y)); //vavy
